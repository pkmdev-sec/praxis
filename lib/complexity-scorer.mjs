...
/**
 * P1 ENHANCEMENT: Multi-signal scoring - analyzes multiple dimensions beyond keywords.
 * Provides granular breakdown of complexity signals for better transparency.
 * @param {string} prompt - The user prompt to analyze.
 * @returns {{
 *   totalScore: number,
 *   breakdown: {
 ...
 * }}
 */
export function scoreWithMultiSignal(prompt) {
  try {
    if (prompt === null || prompt === undefined || prompt === '') {
      return {
        totalScore: 1,
        ...
        signals: { keywords: [], length: 0, questionType: 'unknown', matchedTier: null, wordCount: 0 },
        taskType: TASK_TYPES.QUESTION,
      };
    }
    if (typeof prompt !== 'string') {
      throw new Error(`Invalid prompt: expected string, got ${typeof prompt}`);
    ...
    // Detect code references (backticks, file paths, function names)
    const hasCodeReferences = /`[^`]+`|\.js|\.py|\.java|\.go|function\(|=>|class\s+\w+/i.test(prompt);

    // Count technical terms
    const technicalTerms = [
      'api', 'database', 'algorithm', 'cache', 'async', 'promise', 'thread',
      'mutex', 'lock', 'query', 'index', 'schema', 'endpoint', 'webhook',
      'authentication', 'authorization', 'encryption', 'ssl', 'tls', 'jwt',
      'oauth', 'rest', 'graphql', 'socket', 'protocol', 'middleware',
    ];
    ...
    const bullets = (prompt.match(/^[\s]*[-*]\s/gm) || []).length;
    let structuralScore = Math.min(10, 1 + codeBlockCount * 2 + (questionCount - 1) * 1.5 + bullets * 0.5);

    // Bonus for technical terms
    structuralScore = Math.min(10, structuralScore + technicalTermCount * 0.3);

    // Bonus for code references
    if (hasCodeReferences) {
      structuralScore = Math.min(10, structuralScore + 1);
    }
    ...
    // Clamp to 1-10 and round
    const totalScore = Math.max(1, Math.min(10, Math.round(blended)));
    ...
    console.error(`Error in scoreWithMultiSignal: ${error.message}`);
    throw error;
  }
}

/**
 * P1 ENHANCEMENT: Analyze prompt for code-specific complexity signals.
 * Useful for determining if a prompt involves code review, debugging, or implementation.
 * @param {string} prompt - The user prompt to analyze.
 * @returns {{
 *   hasCode: boolean,
 *   codeBlockCount: number,
 ...
 * }}
 */
export function analyzeCodeComplexity(prompt) {
  try {
    ...
    const hasCode = codeBlockCount > 0 || hasInlineCode;

    // Extract file references
    const fileMatches = prompt.match(/[\w-]+\.(js|ts|py|java|go|rb|php|cpp|c|h|rs|swift|kt|jsx|tsx|mjs|cjs|vue)/gi) || [];
    const fileReferences = [...new Set(fileMatches)];

    // Extract function references (basic patterns)
    const functionMatches = prompt.match(/\b[a-z_][a-z0-9_]*\(/gi) || [];
    const functionReferences = [...new Set(functionMatches.map(m => m.replace('(', '')))];

    // Determine complexity
    let complexity = 'low';
    if (codeBlockCount > 2 || fileReferences.length > 3 || functionReferences.length > 5) {
      complexity = 'high';
    } else if (codeBlockCount > 0 || fileReferences.length > 0 || functionReferences.length > 2) {
      complexity = 'medium';
    }
    ...
    console.error(`Error in analyzeCodeComplexity: ${error.message}`);
    throw error;
  }
}
